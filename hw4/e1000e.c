...
/* timer callback: timer blinks on/off at rate set by blink_rate */
...
static int dev_open(struct inode *inode, struct file *file) {
	...
	/* show register and blink rate status */
    	led_reg = readl(devs->hw_addr + LED_CNTRL_REG);
    	printk(KERN_INFO "led_reg = 0x%08x blink_rate = %d\n", 
	       led_reg, blink_rate);

	/* start a timer when char device gets opened */
    	mod_timer(&my_timer, HZ/(blink_rate*2) + jiffies);
...
static int dev_probe(struct pci_dev *pdev, const struct pci_device_id *ent) {
	...
	/* make sure insmod blink_rate is good value */
	if(blink_rate < 1) {
		blink_rate = 2;
		printk(KERN_ERR "invalid insmod blink_rate value.. blink_rate = 2\n");
	}
      
	/* start timer for blinking LEDs */ 
